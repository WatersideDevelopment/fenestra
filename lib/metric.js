...
Metric.getSession = function(req) {
	...
	// first use a detector, loaded only one by default?
	// get the session_key (detector specitic)
	// and return it bck to here

	// next, our local job, look up the actual session record
	// if we have one, that is... - we may not.

	//	and if we have a session key, we s and no record
	// we should be storing a new record, and returning that/...

	// what about user
	// what about auth

// if we have a session, and a suser_key, but no user, we should mod and
// save the session

// if this is a new session, we should try and get the userkey...
