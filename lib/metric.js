    ...
    if(sessionKey) {
        SessionShelf.Session.forge({key: sessionKey}).fetch().then(function(session) {
            if (!session) {
                ...
                console.log("no session found (key=="+userKey+")");
                session = {};
        ...
        console.log('not trying to create an empty session');
        callback();
    }
};
...
Metric.getUser = function(req, callback) {
    ...
    console.log("got a userKey: "+userKey);
    if(userKey) {
...
//        console.log('not trying to create an empty user');
        callback();
    }
};
